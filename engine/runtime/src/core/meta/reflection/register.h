...
// 存储类类型信息的TypeInfo
template <typename T>
class TypeInfo<T, Array> : public Singleton<TypeInfo<T, Array>>
{
  public:
    TypeInfo &Register(const std::string &name);
    void AutoRegister();

    const Array &GetInfo() const
    {
        if (!TypeInfo<T, Array>::Instance().saved)
        {
            saved = true;
            TypeInfo<T, Array>::Instance().AutoRegister();
        }
        return info;
    }
  private:
...
class Property_Impl : public Property
{
  public:
    ...
    std::any CallConst(const std::any &a) const override
    {
        using ClassType = property_traits<Ptr>::ClassType;
        if (GetType<ClassType>() != GetOwner())
            throw std::runtime_error("Type mismatch");
        auto &obj = std::any_cast<std::reference_wrapper<const ClassType>>(a).get(); //所有的std::any存储的是引用，这里要解除std::ref的引用
        return std::cref(obj.*accessor);
    }
