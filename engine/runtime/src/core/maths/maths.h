...
static void SerializeQuat(const Any &instance, const Type *typeInfo, YAML::Emitter &out)
{
    const quat obj = instance.As<quat>();
    out << YAML::Flow << YAML::BeginSeq << obj.x << obj.y << obj.z << obj.w << YAML::EndSeq;
}
static void DeserializeQuat(Any &instance, const Type *typeInfo, const YAML::Node &data)
{
...
static CustomSerialize<quat> customSerializeQuat({SerializeQuat, DeserializeQuat});
