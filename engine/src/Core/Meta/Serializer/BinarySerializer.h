...
class BinarySerializer : public ISerializer 
{
  public:
    ...
    const std::vector<uint8> &GetOutput() { return buffer; }
    
    void WritePack(const void *data, uint32 size)
    {
        WriteTag(BinaryTag::Pack);
        WriteRaw<uint32>(size);
        buffer.insert(buffer.end(), reinterpret_cast<const uint8*>(data), reinterpret_cast<const uint8*>(data) + size);
    }

  private:
    void WriteTag(BinaryTag tag)
    {
        buffer.push_back((uint8)tag);
    }

    template<typename T>
    void WriteRaw(const T &val)
    {
        const uint8* bytePtr = reinterpret_cast<const uint8*>(&val);
        buffer.insert(buffer.end(), bytePtr, bytePtr + sizeof(T));
    }
    void WriteData(const void *data, size_t size)
    {
        const uint8* bytePtr = reinterpret_cast<const uint8*>(data);
    ...
    void SerializeFundamental(const AnyRef value, const Fundamental *typeInfo)
    {
        ...
        case UInt8: WriteTag(BinaryTag::UInt8); WriteRaw<uint8>(value.As<uint8>()); break;
        case UInt16: WriteTag(BinaryTag::UInt16); WriteRaw<uint16_t>(value.As<uint16_t>()); break;
        case UInt32: WriteTag(BinaryTag::UInt32); WriteRaw<uint32>(value.As<uint32>()); break;
...
{
  public:
    BinaryDeserializer(const uint8 *data, uint32 size)
        : data(data), size(size)
    {
        if (IsMap())
        {
            ...
            {
                uint32 keyEnd = SkipNode(pos);
                BinaryDeserializer keyDes(data + pos, keyEnd - pos);
                uint32 valueEnd = SkipNode(keyEnd);
                BinaryDeserializer valueDes(data + keyEnd, valueEnd - keyEnd);
                itemStorage.push_back({MoveTemp(keyDes), MoveTemp(valueDes)});
                pos = valueEnd;
            }
        }
        else if (IsSeq())
        {
            ...
            {
                uint32 elemEnd = SkipNode(pos);
                BinaryDeserializer elemDes(data + pos, elemEnd - pos);
                elemStorage.push_back(MoveTemp(elemDes));
                pos = elemEnd;
            }
        }
    }
    ...
    BinaryDeserializer(Buffer buffer)
        : data(buffer.Data<uint8>()), size(buffer.Size<uint8>()) {}
    ...
    virtual std::pair<const uint8 *, uint32> Unpack() const override
    {
        uint32 size = ReadRaw<uint32>(data[1]);
        return { &data[1 + sizeof(uint32)], size };
    }
    ...
    size_t SkipNode(size_t pos) const
    {
        ...
        case BinaryTag::UInt8: return pos + sizeof(uint8);
        case BinaryTag::UInt16:return pos + sizeof(uint16_t);
        case BinaryTag::UInt32:return pos + sizeof(uint32);
        ...
        {
            uint32 len = ReadRaw<uint32>(data[pos]);
            return pos + sizeof(uint32) + len;
        }

        case BinaryTag::Pack:
        {
            uint32 size = ReadRaw<uint32>(data[pos]);
            return pos + sizeof(uint32) + size;
        }

        case BinaryTag::SeqBegin:
        {
            while (pos < size && data[pos] != (uint8)BinaryTag::SeqEnd)
                pos = SkipNode(pos);
            return pos + 1;
        }
        ...
        {
            while (pos < size && data[pos] != (uint8)BinaryTag::MapEnd)
            {
                pos = SkipNode(pos);
                pos = SkipNode(pos);
            }
    ...
    template<typename T>
    const T ReadRaw(const uint8 &data) const
    {
        return *reinterpret_cast<const T*>(&data);
    }

    BinaryTag ReadTag(const uint8 &data) const 
    {
        return (BinaryTag)data;
    }

    void AssertTag(BinaryTag tag, uint32 pos) const
    {
  ...
  private:
    void DeserializeFundamental(AnyRef instance, const Fundamental *typeInfo, uint32 pos, uint32 size) const
    {
        switch (typeInfo->GetKind())
        {
            using enum FundamentalKind;
        ...
        case UInt8: AssertTag(BinaryTag::UInt8, pos); instance.As<uint8>() = ReadRaw<uint8>(data[pos + 1]); break;
        case UInt16: AssertTag(BinaryTag::UInt16, pos); instance.As<uint16_t>() = ReadRaw<uint16_t>(data[pos + 1]); break;
        case UInt32: AssertTag(BinaryTag::UInt32, pos); instance.As<uint32>() = ReadRaw<uint32>(data[pos + 1]); break;
    ...
    void DeserializeString(AnyRef instance, const String *typeInfo, uint32 pos, uint32 size) const
    {
        AssertTag(BinaryTag::String, pos);
        uint32 len = ReadRaw<uint32>(data[pos + 1]);
        instance.As<std::string>() = std::string(data + pos + 1 + sizeof(uint32), data + pos + 1 + sizeof(uint32) + len);
    }

    void DeserializeEnum(AnyRef instance, const Enum *typeInfo, uint32 pos, uint32 size) const
    {
        AssertTag(BinaryTag::Enum, pos);
        typeInfo->SetValue(instance, ReadRaw<int32_t>(data[pos + 1]));
    }

    void DeserializeList(AnyRef instance, const List *typeInfo, uint32 pos, uint32 size) const
    {
    ...
    void DeserializeDict(AnyRef instance, const Dict *typeInfo, uint32 pos, uint32 size) const
    {
        BinaryTag endTag;
    ...
    void DeserializeProperty(AnyRef instance, const std::shared_ptr<Property> &prop, uint32 pos, uint32 size) const
    {
        AnyRef subInstance = prop->Call(instance);
        DeserializeAny(subInstance, prop->GetTypeInfo(), pos, size);
    }

    void DeserializeClass(AnyRef instance, const Class *typeInfo, uint32 pos, uint32 size) const
    {
        AssertTag(BinaryTag::MapBegin, pos);
        ...
        while (ReadTag(data[pos]) != BinaryTag::MapEnd)
        {
            ...
            uint32 valueEnd = SkipNode(pos);
            uint32 valueStart = pos;
    ...
    void DeserializeAny(AnyRef instance, const Type *typeInfo, uint32 pos, uint32 size, AnyRef context = nullptr) const
    {
        if (auto it = customSerializeOps.find(typeInfo); it != customSerializeOps.end())
        {
