...
RenderResourceSchema::RenderResourceSchema(const Path &path)
{
...
void RenderResourceSchema::ParseAliases(const std::vector<std::string> &lines)
{
...
const ShaderReflection::DataType *RenderResourceSchema::ParseDataType(const glslang::TType *type, const glslang::TIntermediate *intermediate)
{
    if (type->isArray())
    {
...
ShaderReflection::UniformFieldLayout RenderResourceSchema::CalculateUniformLayout(const std::string &name, const glslang::TType *type, const glslang::TIntermediate *intermediate, const ShaderReflection::DataType *customType)
{
    if (type->isArray())
    {
...
ShaderReflection::UniformFieldLayout RenderResourceSchema::CalculateArrayLayout(const std::string &name, const glslang::TType *type, const glslang::TIntermediate *intermediate, const ShaderReflection::DataType *customType)
{
    return {};
}
ShaderReflection::UniformFieldLayout RenderResourceSchema::CalculateStructLayout(const std::string &name, const glslang::TType *type, const glslang::TIntermediate *intermediate, const ShaderReflection::DataType *customType)
{
...
ShaderReflection::UniformFieldLayout RenderResourceSchema::CalculateFundamentalLayout(const std::string &name, const glslang::TType *type, const glslang::TIntermediate *intermediate, const ShaderReflection::DataType *customType)
{
    ShaderReflection::UniformFieldLayout layout
    {
        .name = name,
...
RenderResourceDescription RenderResourceSchema::ParseResourceType(const glslang::TIntermSymbol *symbol, const glslang::TIntermediate *intermediate)
{
    ...
    else if (q.storage == glslang::EvqUniform && t.getBasicType() == glslang::EbtSampler)
    {
        ...
        if (sampler.is2D())
        {
            return
            {
                ShaderReflection::GetSampledImageType(ShaderReflection::SamplerType::Texture2D),
                set,
                binding
            };
        }
...
const ShaderReflection::DataType *RenderResourceSchema::ParseArrayType(const glslang::TType *arrayType, const glslang::TIntermediate *intermediate)
{
    return nullptr;
}

const ShaderReflection::DataType *RenderResourceSchema::ParseFundamentalType(const glslang::TType *fundamentalType, const glslang::TIntermediate *intermediate)
{
...
const ShaderReflection::DataType *RenderResourceSchema::ParseStructType(const glslang::TType *structType, const glslang::TIntermediate *intermediate)
{
...
void RenderResourceSchema::TraverseAST(const TIntermNode *node, const glslang::TIntermediate *intermediate)
{
    if (!node) return;
...
void RenderResourceSchema::ParseShaderSchema(const std::string &source)
{
    // 尝试使用片段着色器类型，因为它更通用
...
RenderResourceParameterPath RenderResourceSchema::ParseParameterPath(const std::string &path) const
{
...
const ShaderReflection::DataType *RenderResourceSchema::GetUniformParameterType(const RenderResourceParameterPath &path) const
{
