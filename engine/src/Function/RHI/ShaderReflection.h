...
struct UniformFieldLayout 
{
    ...
    uint32 elemStride = 0; // 仅 Array 使用 
    uint32 arraySize = 1;
    bool isArray = false;

    uint32 matrixStride = 0; // Matrix

    std::vector<UniformFieldLayout> children;
};
...
{ 
  public:
    UniformBlock(const std::string &name, LayoutRule layoutRule, const Struct *structType, UniformFieldLayout layout)
        : ResourceType(name, ResourceTypeCategory::UniformBlock), layoutRule(layoutRule), structType(structType), layout(MoveTemp(layout))
    {
    }

    const Struct *GetType() const { return structType; }

    uint32 GetSize() const { return layout.size; }
