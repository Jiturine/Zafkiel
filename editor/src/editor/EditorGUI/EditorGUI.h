...
class EditorGUI
{
  public:
    ...
    EditorGUI Image(ImTextureRef textureRef, vec2 size, vec2 uv0 = vec2(0.0f, 0.0f), vec2 uv1 = vec2(1.0f, 1.0f))
    {
        if (sameLine) ImGui::SameLine();
        ImGui::Image(textureRef, ImVec2(size.x, size.y), ImVec2(uv0.x, uv0.y), ImVec2(uv1.x, uv1.y));
        sameLine = true;
        return *this;
    }
    EditorGUI SameLine()
    ...
    EditorGUI DragInt(const std::string &label, int &data, std::function<void(void)> onValueChanged = nullptr, float speed = 0.1f)
    {
        if (sameLine) ImGui::SameLine();
    ...
    EditorGUI DragFloat(const std::string &label, float &data, std::function<void(void)> onValueChanged = nullptr, float speed = 0.01f)
    {
        if (sameLine) ImGui::SameLine();
    ...
    EditorGUI DragVec2(const std::string &label, vec2 &data, std::function<void(void)> onValueChanged = nullptr, float speed = 0.01f)
    {
        if (sameLine) ImGui::SameLine();
    ...
    EditorGUI DragVec3(const std::string &label, vec3 &data, std::function<void(void)> onValueChanged = nullptr, float speed = 0.01f)
    {
        if (sameLine) ImGui::SameLine();
    ...
    EditorGUI DragVec4(const std::string &label, vec4 &data, std::function<void(void)> onValueChanged = nullptr, float speed = 0.01f)
    {
        if (sameLine) ImGui::SameLine();
...
{
  public:
    GUITreeNode(uint32 id, ImGuiTreeNodeFlags flags, const std::string &name)
